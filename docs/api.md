...
### Build identifier — `/version.json`

A static asset (not a worker route) generated by `scripts/gen-version.mjs` — run in CI via `npm run build` and locally via the `predev` / `predeploy` npm hooks. The landing and About pages fetch it to show the build identifier. It is gitignored; if absent the footer is simply hidden.

The value is a `git describe` string (last release tag, commits since it, short SHA, `-dirty` for an unclean tree). Where tags aren't reachable (Cloudflare Workers Builds does a shallow clone without them), it falls back to `v<package.json version>-g<sha>`.

```json
{ "version": "v1.1.0-3-g1a2b3c4" }
```
